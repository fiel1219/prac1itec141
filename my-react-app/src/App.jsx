...
function DatabaseTable({ page }) {
  ...
  async function approve(row, requestedRole) { const added = await supabase.from('user_accounts').insert({ full_name: row.full_name, email: row.email, role: requestedRole, status: 'active' }); if (added.error) { setError(added.error.message); return } const changed = await supabase.from('account_approvals').update({ status: 'approved', reviewed_role: requestedRole, reviewed_by: 'Current Admin', reviewed_at: new Date().toISOString() }).eq('id', row.id); if (changed.error) setError(changed.error.message); else setRows(rows.filter(item => item.id !== row.id)) }
  if (isApprovals) return <section className="card page"><div className="cardhead"><div><h2>Admin Approvals</h2><p>Choose a role before approving each account</p></div><Badge>{rows.length} requests</Badge></div>{error&&<div className="connection-error">{error}</div>}<table><thead><tr><th>NAME</th><th>EMAIL</th><th>ASSIGN ROLE</th><th>STATUS</th><th>ACTION</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><b>{r.full_name}</b></td><td>{r.email}</td><td>{r.status==='pending'?<select value={r.reviewed_role || 'Operator'} onChange={e=>setRows(rows.map(x=>x.id===r.id?{...x,reviewed_role:e.target.value}:x))}><option>Admin</option><option>Operator</option></select>:r.reviewed_role}</td><td><Badge tone={r.status==='pending'?'':'neutral'}>{r.status}</Badge></td><td>{r.status==='pending'&&<button className="primary" onClick={()=>approve(r,r.reviewed_role || 'Operator')}>Approve</button>}</td></tr>)}</tbody></table></section>
