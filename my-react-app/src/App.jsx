...
export default function App() {
  ...
  return <div className="app">
    ...
    <div className="user-bar"><div className="user-avatar">{(currentUser?.full_name || 'U').charAt(0).toUpperCase()}</div><div><b>{currentUser?.full_name || 'Loading user...'}</b><small>{currentUser?.email || ''}</small></div><span>{currentUser?.role || 'Operator'}</span><button className="logout" onClick={()=>supabase?.auth.signOut()}>Log out</button></div>
