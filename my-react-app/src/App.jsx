...
import './App.css'
import { fetchShelves, fetchBorrowers, fetchTransactions, fetchAdmins, isSupabaseConfigured, supabase } from './lib/supabase'
...
function DatabaseTable({ page }) {
  ...
  useEffect(() => { const load = page === 'Borrowers' ? fetchBorrowers() : page === 'Transactions' ? fetchTransactions() : page === 'Admins' ? fetchAdmins() : Promise.resolve({ data: [] }); load.then(({ data, error }) => { if (error) setError(error.message); else setRows(data || []); setLoading(false) }) }, [page])
  const isBorrowers = page === 'Borrowers', isAdmins = page === 'Admins'
